...
export const any_client = (id: string, type: string) => {
  ...
  const _ = setTimeout(() => {
    const socket = io(`http://localhost:5000/${id}`);
    ...
    socket.on("connect", () => {
      ...
      socket.emit(
        `discuss_common_logic`,
        `Let's start discuss under common_logic (from client[${type}])?`,
      );

      //...
      socket.on(`discuss_${type}`, (m: string) => {
        console.log(
          `[discuss_${type}:${short}] Client${type} receives \"${m}\"`,
        );
      });
      socket.on(`discuss_common_logic`, (m: string) => {
        console.log(
          `[discuss_common_logic:${short}] Client${type} receives \"${m}\"`,
        );
      });
      socket.on(`discuss_${type}`, (m: string) => {
        console.log(
      ...
      socket.on(`reply_other_topic`, (m: string) => {
        console.log(
          `[reply_other_topic:${short}] Client${type} receives \"${m}\"`,
        );
      });
    });
  }, delay * 1000);
