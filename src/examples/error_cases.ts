...
setTimeout(() => {
  const socket = io(`http://localhost:5000/ErrorService`);
  socket.on("connect", () => {
    const short = socket.id?.substring(0, 4);
    console.log(`\nClient socket [${short}] connected!`);
    socket.emit("nobodyThere", "Are you there?");
    socket.on("nobodyThere", (...d) =>
      console.log("this should no be printed " + d),
    );

    socket.emit("clunckyService", "TicTac");
    socket.on("clunckyService", (...d) =>
      console.log("this should contain an error:\n" + inspect(d)),
    );

    socket.emit("clunckyServiceWithDatumError", "TicToc");
    socket.on("clunckyServiceWithDatumError", (...d) =>
      console.log("this should contain an error Object:\n" + inspect(d)),
    );
  });
}, pullDelay());
