...
export function SocketControllerRegister<
  ...
  return class extends Base {
    constructor(...args: any[]) {
      ...
      self.app.on("connection", (socket: Socket) => {
        if (self.debug)
          console.log(
            `[SocketManager] New connection [${socket.id.slice(0, 4)}]`,
          );
        ...
        if (subMethods) {
          subMethods.forEach((requestName: string, method: string) => {
            if (self.debug)
              console.log(
                `SocketControllerRegister:onConnection[requestName:method] ${requestName}:${method}`,
              );
            const ansEvtName = requestName ?? method;
            socket.on(method, async (...sockArgs: any[]) => {
              if (self.debug) {
                console.log(
                  `ListenTo: incoming event \"${method}\" on socket ${socket.id.slice(0, 4)} <<<<`,
                );
                console.log(`\tdata: ${sockArgs}`);
              }
              let maybeAckCallback: undefined | ((d: any) => any) = undefined;
              try {
                if (sockArgs.length > 0)
                  if (typeof sockArgs[sockArgs.length - 1] === "function")
                    //console.log("ACK callback unwraped");
                    maybeAckCallback = sockArgs[sockArgs.length - 1];
                // Only passing input(s)/ discarding eventual callback to the bound logic
                // Adding the trailer underlying socket
                const maybeResults = await Promise.resolve(
                  (self as any)[method](
                    ...sockArgs.filter((arg) => typeof arg !== "function"),
                    socket,
                  ),
                );
                if (self.debug) {
                  console.log(
                    `ListenTo: outgoing event \"${ansEvtName}\" on socket ${socket.id.slice(0, 4)} <<<<`,
                  );
                  console.log(`\tdata: ${maybeResults}`);
                }

                if (maybeResults !== undefined)
                  if (maybeAckCallback !== undefined)
                    maybeAckCallback(maybeResults);
                  else socket.emit(ansEvtName, maybeResults);
              } catch (e: any) {
                if (self.debug) {
                  console.log(
                    `ListenTo: __ERROR__ emiting it under \"${ansEvtName}\"`,
                  );
...
export abstract class SocketController {
  ...
  constructor(params: SocketControllerParameters) {
    const { socketServer, id = uuid4(), route, debug = false } = params;

    if (!isAbsolute(route as string))
      // in-case use inputed
      throw new SocketControllerCreationError(
        `The route parameter \"${route}\" must be an absolute path`,
      );
